...
static CGFloat const kSNShowDuration = 1.5;
...
+ (instancetype) sharedInstance {
    ...
    dispatch_once(&onceToken, ^{
        sharedHUD = [[MBProgressHUD alloc] initWithFrame: CGRectMake(0, 0, viewWidth(), DHNavigationFrameHeight())];
        sharedHUD.removeFromSuperViewOnHide = YES;
        sharedHUD.margin = 15;
        [[AppDelegate appDelegate].window addSubview: sharedHUD];
    });
...
+ (void) showSuccessWithStatus:(NSString*) status {
    MBProgressHUD *sharedHUD = [MBProgressHUD sharedInstance];
    sharedHUD.mode = MBProgressHUDModeCustomView;
    sharedHUD.customView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"37x-Checkmark.png"]];
    sharedHUD.labelText = status;
    [[AppDelegate appDelegate].window addSubview: sharedHUD];
    [sharedHUD show: YES];
    [sharedHUD hide: YES afterDelay: kSNShowDuration];
}
...
+ (void) showErrorWithStatus:(NSString*) status {
    MBProgressHUD *sharedHUD = [MBProgressHUD sharedInstance];
    sharedHUD.mode = MBProgressHUDModeText;
    sharedHUD.labelText = status;
    [[AppDelegate appDelegate].window addSubview: sharedHUD];
    [sharedHUD show: YES];
    [sharedHUD hide: YES afterDelay: kSNShowDuration];
...
+ (void) showLoadingWithStatus:(NSString*) status {
    MBProgressHUD *sharedHUD = [MBProgressHUD sharedInstance];
    if ([status isNonEmpty]) {
        
        sharedHUD.mode = MBProgressHUDModeIndeterminate;
        sharedHUD.labelText = status;
        [[AppDelegate appDelegate].window addSubview: sharedHUD];
        [sharedHUD show: YES];
    }else {
...
- (void)layoutSubviews {
    ...
    static UILabel *getHeightLabel = nil;
    if (!getHeightLabel) {
        getHeightLabel = [[UILabel alloc] initWithFrame: CGRectMake(0, 0, maxWidth, 0)];
    }
    
    getHeightLabel.frame = CGRectMake(0, 0, maxWidth, 0);
    getHeightLabel.text = label.text;
    getHeightLabel.font = label.font;
    getHeightLabel.numberOfLines = 0;
    [getHeightLabel sizeToFit];
    labelSize = getHeightLabel.size;
#else
#endif
